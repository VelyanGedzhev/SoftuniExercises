    ...
    public class RoutingTable : IRoutingTable
    {
        ...
        public IRoutingTable Map(HttpMethod method, string path, Func<HttpRequest, HttpResponse> responseFunction)
        {
            ...
            if (this.routes.ContainsKey(method)
                && this.routes[method].ContainsKey(path.ToLower()))
            {
                throw new InvalidOperationException($"Route {method} {path}already exists. Multiple routes with the same method and path are not supported.");
            }
