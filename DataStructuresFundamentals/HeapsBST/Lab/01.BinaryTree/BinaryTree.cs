namespace _01.BinaryTree
{
    ...
    public class BinaryTree<T> : IAbstractBinaryTree<T>
    {
        ...
        {
            stringBuilder
                .AppendLine($"{new string(' ', indent)}{current.Value}");

            if (current.LeftChild != null)
            {
                AsIndentedPreOrderDfs(current.LeftChild, indent + 2, stringBuilder);
            }

            if (current.RightChild != null)
            {
                AsIndentedPreOrderDfs(current.RightChild, indent + 2, stringBuilder);
            }
